...
fn main() {
    ...
    let december: NaiveDate = NaiveDate::from_ymd_opt(2024, 12, 1).unwrap();
    ...
    if play_date < december {
        reason = String::from("L'invitation est demandé pour une date avant le 1er décembre 2024");
        println!("{:?}", reason);
    } else {
        println!("L'invitation est demandé pour une date après le 1er décembre 2023: NEXT STEP");
        let check_inivtation_date: i64 =
            play_date.signed_duration_since(invitation_date).num_days();
        if check_inivtation_date <= 0 {
            reason = String::from(
                "La demande d'invitation est faite moins de 1 jours avant la date de jeu",
            );
            answer = Some(0);
            println!("{:?}", reason);
        } else {
            ...
            if result_evenement == false {
                ...
                if result_ic == false {
                    ...
                    let (result_max_invitation, nb_invitations_already_accepted) =
                        check_max_invitations(guest_name.clone(), past_invitations.clone(), 4);
                    if result_max_invitation == false {
                        ...
                        if play_date.weekday() == Weekday::Thu {
                            let (result_invitations_accepted, n_invitations_accepted) =
                                check_invitations_accepted(play_date, past_invitations.clone(), 2);
                            if result_invitations_accepted == false {
                                reason = format!(
                                    "Il y a déjà {} invitations pour le jeudi {}",
                            ...
                            if result_invitations_accepted == false {
                                reason = format!(
                                    "Il y a déjà {} invitations pour le dimanche {}",
                                    n_invitations_accepted, play_date
                                );
                                answer = Some(0);
                                println!("{:?}", reason);
...
fn check_max_invitations(
    guest_name: String,
    past_invitations: DataFrame,
    nb_invitations_max: usize,
) -> (bool, usize) {
    let past_invitations_accepted = past_invitations
        .lazy()
        .filter(
            col("guest_name")
                .eq(lit(guest_name.to_string()))
                .and(col("answer").eq(1)),
        )
        .collect()
        .unwrap();
